...
datatitle <- as.data.frame(read.csv(file="D:\\sourcecode\\smart-search-functional-test\\src\\test\\resources\\testJobtitlesGraphing.csv", header = T, sep = ",", row.names=NULL))
...
ggplot(data=datatitle,aes(responseTime)) +
  ...
  geom_density(alpha = 0.2, fill=6) + scale_x_continuous(limits = c(21,31))
